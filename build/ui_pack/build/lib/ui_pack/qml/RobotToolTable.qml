...
Rectangle{
        ...
        MyButtonText{
            ...
            MouseArea {
                ...
                onClicked: {
                    ...
                    {
                        //发送插入新行信号
                        signal_insert_row_robotToolList(pointName_text.text)
                        console.log("插入新增一行")
                    }
                }
            }
        ...
        MyButtonText{
            ...
            MouseArea {
                ...
                onClicked: {
                    ...
                    signal_refresh_DBToolToTableView()
                }
            }
        }
                                ...
                                delegate: Rectangle{
                                    ...
                                    Button{
                                        ...
                                        onClicked: {
                                            console.log("btn clicked row:",row)
                                            selectRow(row) //选中行操作
                                            tool_edit(name_selected)  //新工具坐标系编辑
                                        }
                                    }
                                }
                            }
    ...
    Component.onCompleted: {
        //点位示教信号
        robotToolCalib.signal_teach_tool_point.connect(signal_teach_tool_point_toolTable)
        //计算新工具平移变换信号
        robotToolCalib.signal_calculate_tool_trans.connect(signal_calculate_tool_trans_toolTable)
        //将新工具平移变换 更新到界面及数据库
        robotToolCalib.signal_update_tool_trans.connect(signal_update_tool_trans_toolTable)
        //计算工具旋转变换
        robotToolCalib.signal_calculate_tool_rotate.connect(signal_calculate_tool_rotate_toolTable)
        //更新工具旋转变换
        robotToolCalib.signal_update_tool_rotate.connect(signal_update_tool_rotate_toolTable)
    }
    ...
    {
        //发送信号 数据库删除一行
        signal_deleteDbRow_robotToolList(name_selected)
    ...
    //工具坐标系 自定义新工具
    function tool_edit(name_select)
    {
        robotToolCalib.name_selected = name_select
        robotToolCalib.visible = true
    }

    //工具坐标系 自定义新工具示教点位
    function teach_point_callBack_toolTable(list_pose_data,index)
    {
        //调用 工具标定窗口 示教返回函数
...
//    }
    ...
    //定义信号
    //删除
    signal signal_deleteDbRow_robotToolList(string name_str)
    //更新
    signal signal_updateDbRow_robotToolList(string name_str,list<string> str_list)
    //数据库新增一行
    signal signal_insert_row_robotToolList(string name_pt)
    //数据库刷新到 tableView信号
    signal signal_refresh_DBToolToTableView()
    //工具坐标系示教信号绑定
    signal signal_teach_tool_point_toolTable(int point_index)
    //计算工具平移变换
